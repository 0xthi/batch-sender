...
contract MultiSender is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function batchSendERC20Deflationary(
        address _token,
        address[] memory _targets,
        uint256[] memory _amounts
    ) public returns (bool success) {
        ...
        uint256 totalSent = 0;
        for (uint256 i = 0; i < _targets.length; i++) {
            uint256 balanceBefore = token.balanceOf(_targets[i]);
            token.safeTransferFrom(msg.sender, _targets[i], _amounts[i]);
            uint256 balanceAfter = token.balanceOf(_targets[i]);
            uint256 received = balanceAfter - balanceBefore;
            totalSent += received;
        }
        emit MultisendToken(totalSent, _token);
        return true;
    }

    function batchSendFixedERC20Deflationary(
        address _token,
        address[] memory _targets,
        uint256 _amount
    ) public {
